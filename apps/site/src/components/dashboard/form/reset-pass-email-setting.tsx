...
type ResetPassEmailSettingProps = {
  handleSubmit: any;
...
const ResetPassEmailSetting = ({
  ...
  errors,
  register,
}: ResetPassEmailSettingProps) => {
  const { site_name } = theme_config;
  return (
    <form key={1} onSubmit={handleSubmit(onConfirmationEmailSubmit)}>
      <div className="grid gap-7 sm:flex sm:gap-10 pb-4">
        {/* Sender Name */}
        <label className="w-full sm:w-2/4" htmlFor="">
          <p className="text-xs text-themeDark font-normal pb-2">Sender Name</p>
          <input
      ...
      <div className="w-full pb-4">
        <p className="text-xs text-themeDark font-normal pb-2">Subject</p>
        <input
          className="w-full border border-gray h-12 rounded px-4 focus:outline-none"
          type="text"
      ...
      <div>
        <button
          ...
          className={`text-white  shadow-themePrimary flex justify-center items-center gap-3 !py-3.5 px-4 rounded-lg ${
        ...
        >
          {/* Save &amp; Change */}
          {isSubmitting ? "Please wait..." : "Save & Change"}
...
export default ResetPassEmailSetting;
