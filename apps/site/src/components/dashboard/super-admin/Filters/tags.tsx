...
const Tag = ({ data }: { data: any }) => {
  ...
  const addTags = (value: any) => {
    if (data.includes(capitalize(value.type))) {
      setError("type", {
        type: "manual",
        message: "This Tags already exists!",
      });
    } else {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        <h3 className="text-xxs">Creat Tags</h3>
        {/* Add job Tag form */}
        ...
        >
          {processing ? "Please wait..." : "Add Tag"}
        ...
        authAxios({
          ...
          .then(() => {
            mutate("/admin/filters/retrives").then(() => {
              ...
              addToast("Tags Deleted Successfully", {
                appearance: "success",
                autoDismiss: true,
              });
            });
  ...
  return (
    <section className="rounded-lg bg-themePrimary/10 min-h-[550px] h-full">
      ...
      <div className="p-4 overflow-y-auto overflow-x-hidden h-[550px] !pb-10">
        <h3 className="text-xs font-semibold mb-7">Tags</h3>
        {filter.length > 0 ? (
          <>
