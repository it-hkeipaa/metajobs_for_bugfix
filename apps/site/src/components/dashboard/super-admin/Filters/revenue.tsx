...
const Revenue = ({ data }: { data: any }) => {
  const { mutate } = useSWRConfig();
  const [Revenues, setRevenues] = React.useState([]) as any;
  ...
  const addRevenues = (value: any) => {
    if (data.includes(capitalize(value.type))) {
      setError("type", {
        type: "manual",
        message: "This Revenues already exists!",
      });
    } else {
      if (!Revenues.includes(capitalize(value.type))) {
        setRevenues([...Revenues, capitalize(value.type)]);
        reset();
      } else {
        setError("type", {
          type: "manual",
  ...
  const addAllRevenues = () => {
    setProcessing(true);
    const newRevenues = [...Revenues, ...data];
    if (!_.isEqual(newRevenues, data)) {
      try {
        authAxios({
          method: "POST",
  ...
  const removeRevenues = (index: any) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        <h3 className="text-xxs">Creat Revenues</h3>
        {/* Add job Revenue form */}
        <form onSubmit={handleSubmit(addRevenues)}>
          <div
            className={`mt-3 w-full border-2 ${
              errors && errors.type ? "border-red-400" : "border-gray"
            } rounded-lg flex flex-wrap gap-3 px-2 py-1 items-center`}
          >
            {Revenues.length > 0 && (
              <>
                {Revenues.map((tag: any, index: any) => (
                  <span
                    key={index}
        ...
        authAxios({
          ...
          .then(() => {
            mutate("/admin/filters/retrives").then(() => {
              ...
              addToast("Revenues Deleted Successfully", {
                appearance: "success",
                autoDismiss: true,
              });
            });
          ...
          .catch(() => {
            addToast("Error Deleting Revenues", {
              appearance: "error",
              autoDismiss: true,
            });
          });
  ...
  return (
    <section className="rounded-lg bg-themePrimary/10 min-h-[550px] h-full">
      ...
      <div className="p-4 overflow-y-auto overflow-x-hidden h-[550px] !pb-10">
        <h3 className="text-xs font-semibold mb-7">Revenues</h3>
        {filter.length > 0 ? (
          <>
            {_.map(filter, (tag, index) => (
              <div
                ...
                key={index}
              >
                <h3 className="text-themeLight text-xs">{tag}</h3>
                <div
          ...
          <p className="text-xs text-red-400 p-2">No data found! ☹️</p>
        )}
      </div>
    </section>
  );
  ...
  return (
    <div className="relative">
      <form onSubmit={searchHandler}>
        <button type="submit" className="absolute left-3 !top-3 cursor-pointer">
