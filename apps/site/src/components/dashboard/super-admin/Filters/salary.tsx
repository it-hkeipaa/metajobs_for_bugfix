...
const Salary = ({ data }: { data: any }) => {
  ...
  const addSalary = (value: any) => {
    if (data.includes(capitalize(value.type))) {
      setError("type", {
        type: "manual",
        message: "This Salary already exists!",
      });
    } else {
      if (!Salary.includes(capitalize(value.type))) {
        setSalary([...Salary, capitalize(value.type)]);
        reset();
      } else {
        setError("type", {
          type: "manual",
  ...
  const addAllSalary = () => {
    ...
    if (!_.isEqual(newSalary, data)) {
      try {
        authAxios({
          method: "POST",
          url: `/admin/filters/retrives`,
          data: {
            avarageSalary: newSalary,
          },
        }).then((res) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        <h3 className="text-xxs">Creat Salary</h3>
        {/* Add job Salary form */}
        <form onSubmit={handleSubmit(addSalary)}>
          <div
    ...
    sweetAlert({
      ...
      if (willDelete) {
        authAxios({
          ...
          url: `/admin/filters/retrives`,
          data: {
            avarageSalary: newData,
          },
        })
          .then(() => {
            mutate("/admin/filters/retrives").then(() => {
              setFilterData(newData);
              addToast("Salary Deleted Successfully", {
                appearance: "success",
                autoDismiss: true,
              });
            });
          ...
          .catch(() => {
            addToast("Error Deleting Salary", {
              appearance: "error",
              autoDismiss: true,
            });
          });
  ...
  return (
    <section className="rounded-lg bg-themePrimary/10 min-h-[550px] h-full">
      ...
      <div className="p-4 overflow-y-auto overflow-x-hidden h-[550px] !pb-10">
        <h3 className="text-xs font-semibold mb-7">Salary</h3>
        {filter.length > 0 ? (
          <>
